...
import DeletedJobHeader from 'src/pages/apps/invoice/list/DeletedJobHeader'
import Head from 'next/head'
import AppSink from 'src/commonExports/AppSink'

const DeletedJobDetails = () => {
  ...
  const fetchData = async () => {
    const query = `query MyQuery {
        listJobsNew5AABS(filter: {Deleted: {eq: true}}) {
          items {
            AdditionChargeId
            AdditionalChargePrice
            AdditionalChargeRate
    ...
    ApiClient.post(`${AppSink}`, { query }, { headers })
      .then(res => {
        setEmployee(res.data.data.listJobsNew5AABS.items)
  ...
  const handleRestore = async () => {
    try {
      const deletePromises = selectedRows.map(async rowId => {
        ...
        if (selectedRow) {
          const query = `mutation my {
            updateJobsNew5AAB(input: {ID: ${selectedRow.ID}, Deleted: false}) {
              AdditionChargeId
              AdditionalChargePrice
              AdditionalChargeRate
              AssignTo
...
export default DeletedJobDetails
