...
const EcommercePopularProducts = () => {
  ...
  const fetchProductName = async () => {
    try {
      const res = await ApiClient.post(`/getpopularproducts`)
  ...
  return (
    <Card style={{ height: '100%' }}>
      <CardHeader
        ...
        subheader='Top 5 products by sales'
        // action={
        //   <OptionsMenu
        ...
        {allProduct.map((item: DataType, index: number) => {
          return (
            ...
            >
              ...
              {item.imgSrc ? (
    <img width={46} src={item.imgSrc} alt={item.ProductName} style={{ marginRight: '16px' }} />
  ) : (
    <div
      style={{
        width: '46px',
