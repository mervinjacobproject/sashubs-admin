...
const SeoPerformance = ({ seoScoreData }: any) => {
  ...
  const options: ApexOptions = {
    ...
    colors: [getColorByPercentage(validatedPercentage())], // Dynamically set color based on percentage
    states: {
      hover: {
        filter: { type: 'none' }
      },
  ...
  return (
    <Card>
      <CardContent style={{ textAlign: 'center' }} className='score-card'>
        <Typography variant='h6'>{seoScoreData?.label}</Typography>
        ...
        <Typography variant='body2' sx={{ textAlign: 'center', color: 'text.disabled' }}>
          {/* {seoScoreData?.score || 0} out of {seoScoreData?.score?.total_tests || 30} */}
          {isDataAvailable
            ? `${seoScoreData?.score || 0} out of ${seoScoreData?.total_tests || 30}`
            : 'Analyzing...'}
        </Typography>
      </CardContent>
    </Card>
