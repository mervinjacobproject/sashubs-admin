...
const AnalyticsData = ({ seoScoreData }: any) => {
  ...
  const validatedPercentage = () => {
    // const percentage = Math.floor((seoScoreData?.score?.pass_count / seoScoreData?.score?.total_tests ) * 100)
  ...
  return (
    <Card style={{ textAlign: 'center' }} className='score-card'>
      <CardContent>
        <Typography variant='h6'>Analytics and Data Tracking</Typography>
        ...
        <Typography variant='body2' sx={{ textAlign: 'center', color: 'text.disabled' }}>
          {/* {seoScoreData?.score || 0} out of {seoScoreData?.score?.total_tests || 5} */}
          {isDataAvailable
            ? `${seoScoreData?.score || 0} out of ${seoScoreData?.score?.total_tests || 5}`
            : 'Analyzing...'}
        </Typography>
