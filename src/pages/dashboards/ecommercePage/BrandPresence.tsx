...
const BrandPrensence = ({ seoScoreData }: any) => {
  ...
  return (
    <Card>
      <CardContent style={{ textAlign: 'center' }} className='score-card'>
        <Typography variant='h6'>Brand Presence and Trust</Typography>
        ...
        <Typography variant='body2' sx={{ textAlign: 'center', color: 'text.disabled' }}>
          {/* {seoScoreData?.score || 0} out of {seoScoreData?.score?.total_tests || 10} */}
          {isDataAvailable
            ? `${seoScoreData?.score || 0} out of ${seoScoreData?.score?.total_tests || 10}`
            : 'Analyzing...'}
        </Typography>
