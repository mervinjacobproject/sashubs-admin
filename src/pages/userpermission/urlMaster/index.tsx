...
const UserRoles = () => {
  ...
  const handleDeleteConfirm = () => {
    const endPoint = 'api/userspermission/deleteurlmaster' as string
        ...
        return (
          ...
          >
            <Box sx={{ display: 'flex', flexDirection: 'column' }}>
              <Tooltip title={`${row.MasterName}`}>
                <Typography>{row.MasterName}</Typography>
              </Tooltip>
            </Box>
          </Box>
        )
  ...
  function applyFilters(status: any, PCName: any, ParentMaster: any) {
    setActiveStatus(status != 2 ? status : '')
    setstatusFilter(status == 2 ? '' : `${status == 1 ? true : false}`)
    setRole(PCName)
    setParentMaster(ParentMaster)
  }
  ...
  const fetchData = () => {
    ...
    const apiParams: any = {
      MasterName: Role,
      MainMasterName: parentMaster,
      Status: statusFilter
    }
    const paramString = new URLSearchParams(apiParams).toString()
    const endpoint = 'api/userspermission/readurlmaster'
  ...
  useEffect(() => {
    fetchData()
  }, [activeStatus, statusFilter, Role, parentMaster])
          ...
          <Tooltip title='Edit'>
            <DrawerComponent width='530px' anchor='right' onOpen={handleOpenDrawer} buttonLabel='Edit'>
              <UrlMasterform
                fetchData={fetchData}
                rowData={row}
                chargeList={[{ Id: 0, MasterName: 'Parent Master' }, ...dataList]}
              />
            </DrawerComponent>
          </Tooltip>
