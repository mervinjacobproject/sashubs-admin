  ...
  function getAllMaster() {
    ...
    ApiLoginClient.get(endPoint + '?' + paramString)
      ...
      .catch((err: any) => {
        console.log('something went wrong', err)
        setIsTableLoading(false)
        if (err.response?.data.message == 'Category not found') {
          setAllMasters([])
        }
      })
  }
    ...
    ...defaultColumns,
    {
      ...
      renderCell: (row: any) => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <Tooltip title='Edit'>
            <DrawerComponent width='750px' anchor='right' onOpen={handleOpenDrawer} buttonLabel='Edit'>
              <RoleForm fetchData={fetchData} rowData={row} chargeList={dataList} allMasters={allMasters} />
            </DrawerComponent>
          </Tooltip>
        </Box>
      )
