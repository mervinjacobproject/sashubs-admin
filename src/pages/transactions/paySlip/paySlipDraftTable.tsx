...
import PaySlipDraftDropdown from './payslipDraftDropdown'
import CustomModal from 'src/pages/components/ReusableComponents/Modal/modal'
import { Button } from '@mui/material'
import Link from 'next/link'
import { Tooltip } from '@mui/material'
import PaySlipStatusDropdown from './payslipStatusDropdown'
import CustomTextField from 'src/@core/components/mui/text-field'
import PaySlipDraftHeader from 'src/pages/apps/invoice/list/payslipDraftHeader'
import AppSink from 'src/commonExports/AppSink'
import jsPDF from 'jspdf'
import 'jspdf-autotable'
import PrintUrl from 'src/commonExports/printUrl'

const PaySlipDraftTable = ({handledraft,handlePaid,handleNotPaid}:any) => {
  const [selectedRows, setSelectedRows] = useState<any>([])
  const [modalOpenSend, setModalOpenSend] = useState(false)
  const [allEmployee, setAllEmployee] = useState([])
  const [allNewData, setAllNewData] = useState([])
  const [alltablevalue, setalltablevalue] = useState([])
  ...
  const fetchData = async () => {
    try {
      ...
      setRowId(response.data.data.listPaySlip5AABS.items[0].ID)
  ...
  const defaultColumns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: ({ row }: any) => {
        ...
        return <PaySlipDraftDropdown fetchData={fetchData}
        handledraft={handledraft}
        handleNotPaid={handleNotPaid}
        handlePaid={handlePaid}
         rowId={row.ID} defaultVal={defaultDraft} />
      }
    }
  ]
        ...
        <>
          ...
          {/* <Tooltip title='Mail'>
            <div>
              <IconButton
                onClick={() => {
                  handleSendClick(row.ID, row.Employee)
                }}
  ...
  return (
    <>
      <DatePickerWrapper>
        <Card>
          <PaySlipDraftHeader setAllEmpName={setAllEmpName} setAllDate={setAllDate} stateValue={stateValue} />
          <DataGrid
            autoHeight
            pagination
            rowHeight={62}
...
export default PaySlipDraftTable
