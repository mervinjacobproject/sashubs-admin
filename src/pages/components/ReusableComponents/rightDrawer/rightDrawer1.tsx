...
const DrawerComponent: React.FC<DrawerComponentProps> = ({
  ...
  const list = () => (
    <Box
      sx={{
        width: width || (anchor === 'top' || anchor === 'bottom' ? 'auto' : '1216px'),
        // background: '#2f2b3d0f',
        background: backColor(),

        height: router.pathname === '/settings/auth' ? 'full' : '100vh'
      }}
      role='presentation'
      onClick={event => {
  ...
  return (
    <React.Fragment>
      ...
      <Drawer
        anchor={anchor}
        open={open || textBoolean ? true : false}
        onClose={() => toggleDrawer({} as React.MouseEvent, false)}
      >
        {list()}
      </Drawer>
    </React.Fragment>
