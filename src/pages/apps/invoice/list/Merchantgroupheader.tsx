...
const CustomerGroupName = ({
  ...
  //const [merchantName, setMerchantName] = useState<string>(''); // Assuming customer is a string
  ...
  const handleDesignationClick = (GroupName: any, id: any) => {
    setSelectedgroupName({ OwnerName: GroupName, id: id });
    onFetchData();
    setMerchantName(GroupName);
    handleEmployeeGroupClose();
    fetchEmployeeGroup();
    setgroupNameSearch('');
  }
  ...
  const onSubmitForm = (data: any) => {
    const empGroupExists = getEmployeeGroup?.some(
      (item: any) => item.OwnerName.toLowerCase() === data.OwnerName.toLowerCase()
    );
    if (empGroupExists) {
      toast.error('OwnerName Name already exists');
      return;
    }


    ApiClient.post(
      `/createmerchant?OwnerName=${data.OwnerName}`
    )
      .then(res => {
        ...
        toast.success('OwnerName Name Added Successfully');
        handleCloseModal();
        onFetchData();
        fetchEmployeeGroup();
        }
    ...
    <>
      <Grid container sx={{ width: '100%' }} spacing={4}>
        <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'start' }}>
          <Typography sx={{ fontWeight: 600, fontSize: '17px' }}>Select a Merchant</Typography>
        </Grid>
        <Grid item xs={12}>
          <Divider />
        </Grid>
        ...
        <Grid item xs={12}>
          <div style={{ height: '300px', overflow: 'auto' }}>
            {getEmployeeGroup?.length === 0 ? (
              <Alert severity='error'>No Merchant Name available</Alert>
            ) : (
              getEmployeeGroup?.map((i: any) => (
                <React.Fragment key={i.id}>
                  <Grid
                    ...
                    onClick={() => handleDesignationClick(i.OwnerName, i.id)}
                  >
                    <Typography>{i.OwnerName}</Typography>
                  </Grid>
                  <Grid>
                    <Divider />
                  </Grid>
      ...
      >
        ...
        <Typography sx={{ fontWeight: 600, fontSize: '16px', paddingY: 3 }}>Add a Merchant Name</Typography>
        <Divider />
        <br/>
                ...
                render={({ field: { value, onChange },fieldState: { error } }) => (
                  <>
                    ...
                    sx={{ width: '100%', mb: 3 }}
                      ...
                      fullWidth
                      value={value || ''}
                      label={<div>Owner Name <Typography variant='caption' color='error'>*</Typography></div>}
